...
Mobile package authority for the Howl project family.

This repo starts Android-first because Howl's Android terminal needs a real
...
- `howl-pm`, the user-facing mobile package CLI
- `howl-pm-admin`, the backend/admin tool for manifests, archives, and release
  publishing
...
- Howl terminal rendering
- Howl PTY/session runtime
- Android Java/Kotlin lifecycle or UI integration
- iOS app runtime policy
- package execution inside mobile apps
- the core Howl Zig codebase

Those stay in `howl` or platform-native app repos.
...
- `../howl`
- Android package name: `howl.term`
- Android prefix: `/data/data/howl.term/files/usr`
...
go run ./cmd/howl-pm-admin help
go run ./cmd/howl-pm-admin contract -platform android
go run ./cmd/howl-pm-admin android-dev-manifest
go run ./cmd/howl-pm-admin android-prefix-archive -hardcoded-policy audit
go run ./cmd/howl-pm-admin android-dev-snapshot-release -dry-run
go run ./cmd/howl-pm-admin android-product-candidate-probe
go run ./cmd/howl-pm-admin android-product-candidate-materialize
go run ./cmd/howl-pm-admin validate examples/android-dev.manifest.json
```
...
`howl-pm-admin` is the backend/admin tool. It owns provider snapshotting,
manifest generation, prefix archive production, validation, and dev snapshot
publishing. It is not the product shell command.

Android dev prefix archives now include `usr/bin/howl-pm` and an install stamp,
so `howl-pm doctor` and `howl-pm list-available` work inside the staged
...
pinned `.deb` payloads, extracts only the upstream `usr/` payload tree, and
...
`android-test-binary` (`howl-android-catalog-smoke`) for catalog-mode validation,
creates a tag, and
...
`howl-pm` is the product-facing package command intended to run inside the Howl
...
1. Decide whether product Android packages come from the current `termux-main`
   provider, a Howl-owned provider, or a controlled mirror/fork. Decision space
   and interim dev authority: `app_architecture/ANDROID_PRODUCT_PROVIDER_DECISION.md`.
   Howl runtime staging contract: `app_architecture/HOWL_PM_ARTIFACT_CONSUMER.md`.
2. Remove or formally own remaining compiled-in Termux prefix assumptions for
   product archives.
3. Add real on-device `howl-pm install/remove/upgrade` semantics after the
   product provider decision.
4. Add iOS artifact-contract notes only when a concrete iOS consumer exists.

## Related Projects

- `howl`: runtime consumer for Android terminal/userland artifacts.
- `howl-tree-sitter`: grammar-pack producer for Howl-family consumers.
