...
| `project` | Must be `howl-pm` for artifacts from this repo |
...
- For bootstrap / prefix materialization: the single **`android-prefix-archive`**
  artifact selected by the same cardinality rule as `howl-pm` (exactly one per
...
### `android-prefix-archive` metadata emitted today (`howl-pm-admin`)

The following keys are written on **`android-prefix-archive`** artifacts produced
by `howl-pm-admin android-prefix-archive` / dev snapshot releases (authoritative
...
| `zide_pm_cli` | Whether `usr/bin/howl-pm` was bundled (`included` when set) |
...
- Extract **`usr/`** tree into the app-private prefix layout expected by
  `metadata.prefix` / deployment policy (same shape as `howl-pm` extract).
...
- Treat `android-termux-deb` artifacts as direct runtime install units (those
  are **host-side production inputs** for `howl-pm-admin`).
