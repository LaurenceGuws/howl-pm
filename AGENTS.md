# Agent Workflow (Howl PM)

This repo is the mobile package authority for Howl-family products.

Naming and ownership are strict:

- `howl-pm` is the product-facing package CLI.
- `howl-pm-admin` is the backend/admin tool.
- `howl-pm` is the repo/module/project identity.
...
- Do not put Howl app runtime code here.
...
- Do not treat provider internals as the Howl product surface.
- Do not describe dev snapshot prereleases as product/official releases.
- Do not let docs or commands imply that `howl-pm-admin` is the user-facing
  shell command; that role belongs to `howl-pm`.
